...
public class MinhaBST<Key extends Comparable<Key>, Value>{
    ...
    public int eagerHeight;        // the height of the BST
    ...
    private Value getCounter(Node x, Key key) {
        ...
        getCount++;
        if      (cmp < 0) return getCounter(x.left, key);
        else if (cmp > 0) return getCounter(x.right, key);
        else              return x.val;
    }

    private double averageSearchMiss() {
        double steps = 0;
